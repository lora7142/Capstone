      ...
      ${state.items
        .map(item => {
          ...
          <table class="reportStyle">
            ...
            <tr><td>Required Maintenance: </td><td>${
              item.requiredMaintenance
            }</td></tr>
            <tr><td>Frequency of Maintenance: </td><td>${
              item.frequencyOfMaintenance
            }</td></tr>
            <tr><td>Maintenance Date: </td><td>${item.maintenanceDate}</td></tr>
            <tr><td>Last Maintenance Dates: </td><td>${
              item.listOfPastMaintenanceDates
            }</td></tr>
            <tr><td>Last Maintenance Type: </td><td>${item.lastMaintenanceType.join(
              ", "
            )}</td></tr>
            <tr><td>Parts Replaced: </td><td>${item.partsReplaced.join(
              ", "
            )}</td></tr>
            <tr><td>Parts Acquired From Business Name: </td><td>${
              item.partsAcquiredFromBusinessName
            }</td></tr>
            <tr><td>Parts Acquired From Business URL: </td><td>${
              item.partsAcquiredFromBusinessURL
            }</td></tr>
            <tr><td>Item Manual URL: </td><td>${item.manualURL}</td></tr>
            <tr><td>Date Acquired: </td><td>${item.dateAcquired}</td></tr>
            <tr><td>Required Secondary Item: </td><td>${
              item.requiredSecondary
            }</td></tr>
            <tr><td>Secondary Related Item: </td><td>${
              item.secondaryRelated
            }</td></tr>
            <tr><td>Secondary Maker: </td><td>${item.secondaryMaker}</td></tr>
            <tr><td>Secondary Model #: </td><td>${
              item.secondaryModelNumber
            }</td></tr>
            <tr><td>Secondary Serial #: </td><td>${
              item.secondarySerialNumber
            }</td></tr>
            <tr><td>Secondary Attachments: </td><td>${
              item.secondaryAttachments
            }</td></tr>
            <tr><td>Notes: </td><td>${item.notes}</td></tr>
            <tr>
              <td><div class="editButton" data-id="${item._id}" data-name="${
            item.name
          }" />Edit Item</div></td>
              <td><div class="deleteButton" data-id="${item._id}" data-name="${
            item.name
          }" />Delete Item</div><td>
            </tr>
            </table>
